...
def _adjust_arrow_endpoints(coords, city_levels, gap_multiplier, dpp):
    """
    Adjust arrow start/end points based on city anchor radii and gap multiplier.

    Args:
        coords: List of [lon, lat] coordinates (will be copied, not modified)
        city_levels: Dict mapping city names to their levels
        gap_multiplier: Multiplier for anchor radius (e.g., 2.0, 3.0, 4.0)
        dpp: Degrees per point

    Returns:
        New list of adjusted coordinates
    """
    # Make a deep copy
    adjusted = [list(c) for c in coords]

    # This function is called with raw coords + names stored elsewhere
    # We need the caller to pass in the city names, so let's return early for now
    # The actual adjustment happens in collect_arrow_candidates
    ...
    for idx, item in enumerate(campaigns):
        ...
        for gap_mult in gap_multipliers:
            ...
            if radius1 > 0:
                ...
                adjusted_coords[0] = _offset_point_toward_internal(
            ...
            if radius2 > 0:
                ...
                adjusted_coords[-1] = _offset_point_toward_internal(
...
def collect_campaigns(gazetteer, manifest, placement_manager):
    ...
    DEPRECATED: Use collect_arrow_candidates() + collect_campaign_labels() instead
    for two-pass resolution with arrow gap adjustment.
